  ...
  <div v-if="showColumnModal" class="modal-overlay" @click="$emit('close')">
    <div class="modal-content" @click.stop>
      ...
      <form @submit.prevent="$emit('add-column')" class="space-y-4">
        ...
        <div v-if="!serverConnected" class="bg-yellow-50 p-3 rounded-lg border border-yellow-200">
          <div class="flex items-center">
            <svg class="w-5 h-5 text-yellow-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
            </svg>
            <span class="text-yellow-800 text-sm font-medium">Server Disconnected</span>
          </div>
          <p class="text-yellow-700 text-xs mt-1">
            Column will be added locally only. Changes will not be saved to the server.
          </p>
        </div>
